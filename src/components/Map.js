...
const MainMap = () => {
  ...
  const [naviInformation, setNaviInformation] = useState([]);
  const [showStartNav, setShowStartNav] = useState(true);

  console.log("rerender ?", naviInformation);
  ...
  const handleLoad = (map) => {
    ...
    const startAutocomplete = new window.google.maps.places.Autocomplete(document.getElementById("Start"), {
      types: ["establishment"],
    ...
    const endAutocomplete = new window.google.maps.places.Autocomplete(document.getElementById("End"), {
      types: ["establishment"],
      componentRestrictions: { country: ["TH"] },
      fields: ["place_id", "geometry", "name"],
    });
  ...
  const startNavigation = () => {
      ...
      (result, status) => {
        result.routes[0].legs.forEach((leg) => {
          leg.steps.forEach((step) => {
            ...
            obj.travelMode = tmode;
            const transit = step.transit;

            if (transit != null) {
              const arrive = transit.arrival_stop.name;
              obj.arrival = arrive;
    ...
    console.count("startNavigation");
    setNaviInformation(resultNav);
  };

  useEffect(() => {
    console.log("change", naviInformation);
  }, [naviInformation]);
  ...
  onLoad={handleLoad}>
      <center>
        {showStartNav ? (
          <div className="input-container relative p-4 top-2 w-fit " style={{ zIndex: "100", backgroundColor: "white", borderRadius: "10px" }}>
            ...
            <button
              className="relative flex flex-nowrap mt-2 md:m-3 p-2 break-all text-xs md:text-base"
      ...
      {showDetail && naviInformation.length > 0 ? (
        ...
        >
          ...
          {naviInformation.map((item, index) => (
            <div
          ...
          >
            ...
            {item.travelMode === "TRANSIT" ? (
              <h1>{item.instructions} <br />
              Line: {item.Line} <br />
              Duration: {item.duration} <br />
              Stop at {item.arrival} </h1>
            ) : (
              <h1>{item.instructions} <br />
              Duration: {item.duration}</h1>
            )}
          </div>
          ))}
        </div>
